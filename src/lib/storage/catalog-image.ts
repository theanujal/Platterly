...
/**
 * Shared by MenuItem/Menu/EventType image uploads — same 2MB PNG/JPG rule
 ...
 * folder, since both the `menus` and `events` modules use it — matches this
 * project's "src/lib/ = cross-cutting platform services" convention.
 */
export async function uploadCatalogImage(
  organizationId: string,
  kind: "items" | "menus" | "event-types",
  file: File,
): Promise<string> {
